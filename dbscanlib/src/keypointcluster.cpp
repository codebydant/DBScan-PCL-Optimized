...
void KeypointCluster::set_values(int y_size, Eigen::Matrix3f e_vecs, Eigen::Vector3f e_vals, float a, float b,
...
KeypointCluster calculate_cluster_descriptor(pcl::PointCloud<pcl::PointXYZRGB>::Ptr Keycloud,
                                             KeypointCluster KeyCluster) {
  ...
  eigenVectorsPCA.col(2) = eigenVectorsPCA.col(0).cross(
      eigenVectorsPCA.col(1));  /// This line is necessary for proper orientation in some cases. The numbers come
                                /// out the same without it, but
  ...
  KeyCluster.set_values(Keycloud->size(), pca.getEigenVectors(), pca.getEigenValues(), minPt.x, minPt.y, minPt.z,
